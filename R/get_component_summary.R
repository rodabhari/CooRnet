...
#' @param summary_type either "component" or "cluster" (default = "component")
#'
...
#'   plot(clusters)
#'
#' @importFrom dplyr %>% group_by summarise mutate top_n arrange group_by_
#' @importFrom urltools suffix_extract domain
#' @importFrom DescTools Gini
#'
#' @export

get_component_summary <- function(output, summary_type="component"){
  ...
  ct_shares_marked.df <- merge(x=ct_shares_marked.df,
                               y=highly_connected_coordinated_entities[,c("name", summary_type)],
                               by.x = "account.url",
                               by.y = "name")
  ...
  summary <- merge(summary_entities, summary_domains, by = summary_type)
  rm(summary_entities, summary_domains)

  summary <- summary %>%
    rowwise() %>%
    mutate(gini.full_domain = DescTools::Gini(prop.table(table(ct_shares_marked.df[ct_shares_marked.df[[summary_type]]==!!as.symbol(summary_type), "full_domain"]))),
           gini.parent_domain = DescTools::Gini(prop.table(table(ct_shares_marked.df[ct_shares_marked.df[[summary_type]]==!!as.symbol(summary_type), "parent_domain"])))) %>%
    ...
    mutate(top.full_domain = paste(top_n(arrange(data.frame(table(ct_shares_marked.df[ct_shares_marked.df[[summary_type]]==!!as.symbol(summary_type), "full_domain"])), desc(Freq)),
                                         n=5, wt="Freq")$Var1, collapse = ", "),
           top.parent_domain = paste(top_n(arrange(data.frame(table(ct_shares_marked.df[ct_shares_marked.df[[summary_type]]==!!as.symbol(summary_type), "parent_domain"])), desc(Freq)),
                                           n=5, wt="Freq")$Var1, collapse = ", "))
  cat("Done!")
